...
import { API_PERMISSION_GROUPS, API_PERMISSION_LABELS, API_TOKEN_PERMISSIONS, DEFAULT_API_PERMISSIONS, type ApiTokenPermission } from "@/lib/api-token-permissions";
...
function TokenDialog({ title, form, update, projects, error, saving, close, save }: { title: string; form: Form; update: React.Dispatch<React.SetStateAction<Form>>; projects: Project[]; error: string; saving: boolean; close: () => void; save: () => void }) {
  ...
  const allPermissionsSelected = API_TOKEN_PERMISSIONS.every((permission) => form.permissions.includes(permission));
  const toggleAllPermissions = () => update((value) => ({
    ...value,
    permissions: allPermissionsSelected ? [] : [...API_TOKEN_PERMISSIONS],
  }));
