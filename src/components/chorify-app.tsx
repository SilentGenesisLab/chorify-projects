...
const navigation = [
  ...
  { label: "任务中心", href: "/tasks", icon: ListChecks, children: ["我的任务", "全部任务"] },
  { label: "需求管理", href: "/requirements", icon: Target },
  { label: "Bug 管理", href: "/bugs", icon: Bug },
  { label: "版本与发布", href: "/versions", icon: Rocket, children: ["版本管理", "发布记录"] },
];
...
const statusStyle: Record<string, string> = {
  "进行中": "bg-blue-50 text-blue-700", "待处理": "bg-slate-100 text-slate-600",
  "待验收": "bg-amber-50 text-amber-700", "已完成": "bg-emerald-50 text-emerald-700",
  "高": "bg-rose-50 text-rose-700", "中": "bg-amber-50 text-amber-700", "低": "bg-slate-100 text-slate-600",
  "正常": "bg-emerald-50 text-emerald-700", "有风险": "bg-rose-50 text-rose-700",
};

function Status({ children }: { children: string }) {
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusStyle[children] || "bg-violet-50 text-violet-700"}`}>{children}</span>;
}

function Avatar({ name, color = "bg-blue-600" }: { name: string; color?: string }) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col border-r border-[#e1e8f2] bg-white transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <nav className="scrollbar-none flex-1 overflow-y-auto px-3 py-5">
        <p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[.14em] text-slate-400">工作空间</p>
        {navigation.map((item) => {
          const Icon = item.icon; const active = route === item.href.slice(1) || (route === "dashboard" && item.href === "/");
          return <div key={item.label} className="mb-1">
            <Link href={item.href} onClick={onClose} className={`flex h-10 items-center gap-3 rounded-xl px-3 text-[14px] font-medium transition ${active ? "bg-[#edf3ff] text-[#2458ce]" : "text-[#5f6c83] hover:bg-slate-50 hover:text-slate-900"}`}>
              <Icon size={18} strokeWidth={1.8}/><span>{item.label}</span>{item.children && <ChevronDown className="ml-auto" size={15}/>} 
            </Link>
            {active && item.children && <div className="ml-8 border-l border-blue-100 py-1 pl-4">{item.children.map(x => <div key={x} className="py-1.5 text-xs text-slate-500">{x}</div>)}</div>}
          </div>;
        })}
...
const tasks = [
  { id:"CP-142", name:"完成项目成员权限矩阵", project:"Chorify Projects", owner:"陈默", priority:"高", status:"进行中", date:"今天" },
  { id:"CP-138", name:"梳理任务提交与验收流程", project:"Chorify Projects", owner:"林舟", priority:"中", status:"待验收", date:"8月28日" },
  { id:"WEB-48", name:"移动端导航适配", project:"官网重构", owner:"周青", priority:"中", status:"进行中", date:"8月30日" },
  { id:"CP-129", name:"整理首版演示数据", project:"Chorify Projects", owner:"苏禾", priority:"低", status:"待处理", date:"9月1日" },
];

function Dashboard() {
  return <div className="space-y-6">
    <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-1 text-sm text-slate-500">2026年8月27日 · 星期四</p><h2 className="text-2xl font-bold tracking-tight text-[#17223b]">下午好，陈默</h2><p className="mt-1 text-sm text-slate-500">3 个项目正在推进，今天有 4 项工作需要关注。</p></div><button className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#376ce7] px-4 text-sm font-semibold text-white shadow-sm shadow-blue-200"><Plus size={17}/>新建项目</button></section>
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {[{l:"进行中的项目",v:"3",s:"本月新增 1 个",i:Layers3,c:"text-blue-600 bg-blue-50"},{l:"我的待办",v:"12",s:"4 项今天到期",i:ListChecks,c:"text-violet-600 bg-violet-50"},{l:"待验收",v:"5",s:"较昨日增加 2 项",i:ClipboardCheck,c:"text-amber-600 bg-amber-50"},{l:"延期风险",v:"2",s:"需要尽快处理",i:AlertTriangle,c:"text-rose-600 bg-rose-50"}].map(x => <div className="card p-5" key={x.l}><div className="flex items-start justify-between"><div><p className="text-sm text-slate-500">{x.l}</p><p className="mt-2 text-3xl font-bold tracking-tight">{x.v}</p></div><div className={`grid size-10 place-items-center rounded-xl ${x.c}`}><x.i size={20}/></div></div><p className="mt-4 text-xs text-slate-400">{x.s}</p></div>)}
    </section>
    <section className="grid gap-5 xl:grid-cols-[1.55fr_1fr]">
      <div className="card overflow-hidden"><div className="flex items-center justify-between border-b border-[#edf1f5] px-5 py-4"><div><h3 className="font-semibold">我的任务</h3><p className="mt-0.5 text-xs text-slate-400">按截止时间排序</p></div><Link href="/tasks" className="text-sm font-medium text-blue-600">查看全部</Link></div><TaskTable compact/></div>
      <div className="card p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">项目状态</h3><p className="mt-0.5 text-xs text-slate-400">当前活跃项目</p></div><MoreHorizontal size={18} className="text-slate-400"/></div><div className="mt-5 space-y-5">{[{n:"Chorify Projects",p:68,s:"正常",c:"bg-blue-500"},{n:"官网重构",p:42,s:"有风险",c:"bg-amber-500"},{n:"数据工作台",p:86,s:"正常",c:"bg-emerald-500"}].map(x => <div key={x.n}><div className="mb-2 flex items-center"><span className="text-sm font-medium">{x.n}</span><span className="ml-auto text-xs text-slate-400">{x.p}%</span><span className="ml-2"><Status>{x.s}</Status></span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${x.c}`} style={{width:`${x.p}%`}}/></div></div>)}</div></div>
    </section>
    <section className="grid gap-5 lg:grid-cols-2"><div className="card p-5"><h3 className="font-semibold">版本进度</h3><div className="mt-4 flex items-center gap-4 rounded-xl bg-[#f7f9fc] p-4"><div className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-600"><Rocket size={21}/></div><div className="min-w-0 flex-1"><div className="flex justify-between"><p className="font-medium">V0.9 协作内测版</p><span className="text-sm font-semibold text-blue-600">72%</span></div><p className="mt-1 text-xs text-slate-400">18 / 25 项已完成 · 计划 9月5日发布</p><div className="mt-3 h-1.5 rounded-full bg-slate-200"><div className="h-full w-[72%] rounded-full bg-blue-500"/></div></div></div></div><div className="card p-5"><h3 className="font-semibold">最近动态</h3><div className="mt-4 space-y-4">{["林舟提交了任务 CP-138 等待验收","苏禾将 Bug CP-27 关联至 V0.9","陈默更新了项目成员权限"].map((x,i)=><div key={x} className="flex gap-3"><div className={`mt-1 size-2 rounded-full ${i===0?"bg-blue-500":i===1?"bg-rose-400":"bg-emerald-500"}`}/><div><p className="text-sm text-slate-600">{x}</p><p className="mt-0.5 text-xs text-slate-400">{i+1} 小时前</p></div></div>)}</div></div></section>
  </div>;
}

function TaskTable({ compact=false }: { compact?: boolean }) {
  return <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead><tr className="border-b border-[#edf1f5] bg-[#fafbfd] text-[11px] uppercase tracking-wide text-slate-400"><th className="px-5 py-3 font-medium">任务</th><th className="px-4 py-3 font-medium">负责人</th><th className="px-4 py-3 font-medium">优先级</th><th className="px-4 py-3 font-medium">状态</th><th className="px-5 py-3 font-medium">截止</th></tr></thead><tbody>{tasks.slice(0,compact?4:undefined).map(x=><tr key={x.id} className="border-b border-[#f0f3f7] last:border-0 hover:bg-slate-50/70"><td className="px-5 py-3.5"><p className="text-sm font-medium">{x.name}</p><p className="mt-0.5 text-xs text-slate-400">{x.id} · {x.project}</p></td><td className="px-4 py-3.5"><div className="flex items-center gap-2"><Avatar name={x.owner}/><span className="text-sm text-slate-600">{x.owner}</span></div></td><td className="px-4 py-3.5"><Status>{x.priority}</Status></td><td className="px-4 py-3.5"><Status>{x.status}</Status></td><td className="px-5 py-3.5 text-sm text-slate-500">{x.date}</td></tr>)}</tbody></table></div>;
}
...
function GenericPage({ route }: { route: string }) {
  ...
  const cards = route==="projects" ? [{n:"Chorify Projects",m:"8 位成员 · V0.9 内测版",p:68,s:"正常"},{n:"官网重构",m:"5 位成员 · V2.1",p:42,s:"有风险"},{n:"数据工作台",m:"4 位成员 · V1.4",p:86,s:"正常"}] : route==="versions" ? [{n:"V0.9 协作内测版",m:"25 项范围 · 9月5日",p:72,s:"进行中"},{n:"V0.8 基础框架",m:"18 项范围 · 8月10日",p:100,s:"已完成"}] : [{n:routeTitles[route]+"示例一",m:"Chorify Projects · 刚刚更新",p:64,s:"进行中"},{n:routeTitles[route]+"示例二",m:"官网重构 · 昨天更新",p:35,s:"待处理"}];
  return <div className="space-y-5"><PageTitle title={routeTitles[route]} subtitle={c.subtitle} action={c.action}/><Filters/>{route==="keys" && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><b>安全提示：</b>密钥只在创建时显示一次。系统仅保存哈希，撤销后立即失效。</div>}<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cards.map((x,i)=><article key={x.n} className="card p-5 transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-start"><div className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-600"><Icon size={21}/></div><button className="ml-auto text-slate-400"><MoreHorizontal size={19}/></button></div><h3 className="mt-5 font-semibold">{x.n}</h3><p className="mt-1 text-xs text-slate-400">{x.m}</p><div className="mt-5 flex items-center gap-3"><div className="h-1.5 flex-1 rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-500" style={{width:`${x.p}%`}}/></div><span className="text-xs font-medium text-slate-500">{x.p}%</span></div><div className="mt-4 flex items-center"><Status>{x.s}</Status><div className="ml-auto flex -space-x-2"><Avatar name={i?"林舟":"陈默"}/><Avatar name={i?"苏禾":"周青"} color="bg-violet-500"/></div></div></article>)}</div></div>;
}
