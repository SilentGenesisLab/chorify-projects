...
import { deliverPendingDeploymentNotifications } from "@/lib/deployment-notifications";
...
export async function POST(request: NextRequest) {
  ...
  const notifications = await deliverPendingDeploymentNotifications();
  return NextResponse.json({ checked: environments.length, healthy: results.filter((item) => item.status === "fulfilled" && item.value.status === "HEALTHY").length, failed: results.filter((item) => item.status === "rejected").length, notifications });
}
