  ...
  return NextResponse.json({
    ...
    projects: projects.map((item) => {
      ...
      return {
        ...
        scope: item.teamId ? "TEAM" : "PERSONAL",
        team: item.team ? { id: item.team.id, name: item.team.name } : null,
        memberCount: item._count.members,
        taskCount: item._count.tasks,
        canManage: projectRole === "OWNER" || projectRole === "MANAGER" || teamRole === "OWNER" || teamRole === "ADMIN",
        canDelete: projectRole === "OWNER" || teamRole === "OWNER" || teamRole === "ADMIN",
        canTransfer: !item.teamId && projectRole === "OWNER",
...
export async function POST(request: NextRequest) {
  ...
  const teamId = input.data.teamId || null;
  if (teamId) {
    const membership = await prisma.teamMember.findUnique({
      where: { teamId_userId: { teamId, userId } },
    });
    if (!membership || !canCreateTeamProject(membership.role)) {
      return NextResponse.json({ error: "只有团队正式成员可以在该团队中创建项目" }, { status: 403 });
    }
  }
  ...
  const project = await prisma.$transaction(async (tx) => {
    const created = await tx.project.create({
      data: {
        ...
        members: {
          create: { userId, role: "OWNER", responsibility: "项目负责人" },
        },
      },
    });
    await tx.auditLog.create({
      data: {
        ...
        metadata: {
          ...
          scope: created.teamId ? "TEAM" : "PERSONAL",
          code,
        },
      },
    });
