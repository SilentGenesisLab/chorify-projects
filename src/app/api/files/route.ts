...
export async function GET(request: Request) {
  ...
  if (user.apiToken && !projectId) return NextResponse.json({ error: "API Key 需要指定项目范围" }, { status: 400 });
  ...
  const [folders, treeFolders, files, usage] = await Promise.all([
    prisma.fileFolder.findMany({ where: { projectId: scopeProjectId, parentId: folderId || null, deletedAt }, include: { project: { select: { name: true } }, creator: { select: { name: true } } }, orderBy: [{ sortOrder: "asc" }, { name: "asc" }] }),
    prisma.fileFolder.findMany({ where: { projectId: scopeProjectId, deletedAt: null }, select: { id: true, projectId: true, parentId: true, name: true, path: true }, orderBy: [{ sortOrder: "asc" }, { name: "asc" }] }),
    prisma.fileAsset.findMany({ where: { projectId: scopeProjectId, folderId: folderId || null, deletedAt }, include: { project: { select: { name: true } }, creator: { select: { name: true } }, currentVersion: true, _count: { select: { versions: true, links: true } } }, orderBy: { updatedAt: "desc" } }),
    storageUsage(),
  ]);
