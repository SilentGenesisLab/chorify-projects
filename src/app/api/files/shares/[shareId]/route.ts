...
export async function DELETE(request: Request, { params }: { params: Promise<{ shareId: string }> }) {
  ...
  const share = await prisma.fileShare.findUnique({ where: { id: shareId } }); if (!share) return NextResponse.json({ error: "分享不存在" }, { status: 404 }); if (!(await fileScopeAccess(user.id, share.projectId, share.creatorId)).canManage) return NextResponse.json({ error: "没有撤销分享的权限" }, { status: 403 });
