...
import { apiTokenAllowsProject, fileScopeAccess, fileUser, hostDiskAllowsUpload, MAX_FILE_SIZE, storageUsage, STORAGE_UPLOAD_STOP } from "@/lib/file-auth";
import { startMultipart } from "@/lib/object-storage";

const schema = z.object({ projectId: z.string().min(1).nullable(), folderId: z.string().nullable().optional(), fileId: z.string().optional(), name: z.string().trim().min(1).max(240), mimeType: z.string().max(200).default("application/octet-stream"), size: z.number().int().positive() });
export async function POST(request: Request) {
  ...
  const permission = await fileScopeAccess(user.id, input.data.projectId); if (!permission.canWrite) return NextResponse.json({ error: "没有上传权限" }, { status: 403 });
  if (user.apiToken && (!input.data.projectId || !apiTokenAllowsProject(user, input.data.projectId))) return NextResponse.json({ error: "API Key 未授权访问该文件范围" }, { status: 403 });
  ...
  const fileId = input.data.fileId || randomUUID(), objectKey = `${input.data.projectId ? `projects/${input.data.projectId}` : "workspace"}/${fileId}/${randomUUID()}`;
  try {
