...
import { apiTokenAllowsProject, fileScopeAccess, fileUser } from "@/lib/file-auth";
...
export async function GET(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  ...
  const file = await prisma.fileAsset.findUnique({ where: { id: fileId }, include: { currentVersion: true } }); if (!file || file.deletedAt || (user.apiToken && (!file.projectId || !apiTokenAllowsProject(user, file.projectId))) || !(await fileScopeAccess(user.id, file.projectId, file.creatorId)).canRead) return NextResponse.json({ error: "文件不存在" }, { status: 404 });
