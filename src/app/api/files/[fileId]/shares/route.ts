...
export async function POST(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  ...
  const file = await prisma.fileAsset.findUnique({ where: { id: fileId } }); if (!file || file.deletedAt) return NextResponse.json({ error: "文件不存在" }, { status: 404 }); if (!(await projectFileAccess(user.id, file.projectId)).canManage) return NextResponse.json({ error: "只有项目管理者可以创建外部分享" }, { status: 403 });
