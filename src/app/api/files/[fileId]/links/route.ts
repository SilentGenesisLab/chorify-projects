...
export async function POST(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  ...
  const file = await prisma.fileAsset.findUnique({ where: { id: fileId } }); if (!file?.projectId || !apiTokenAllowsProject(user, file.projectId) || !(await projectFileAccess(user.id, file.projectId)).canWrite) return NextResponse.json({ error: "未归属项目的文件不能引用项目工作项" }, { status: 403 });
