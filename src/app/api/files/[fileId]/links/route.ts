...
export async function POST(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  ...
  const file = await prisma.fileAsset.findUnique({ where: { id: fileId } }); if (!file || !(await projectFileAccess(user.id, file.projectId)).canWrite) return NextResponse.json({ error: "没有引用文件的权限" }, { status: 403 });
