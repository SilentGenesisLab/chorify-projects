...
import { fileUser, jsonBigInt, projectFileAccess } from "@/lib/file-auth";
...
export async function GET(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  ...
  if (!file || !(await projectFileAccess(user.id, file.projectId)).canRead) return NextResponse.json({ error: "文件不存在" }, { status: 404 });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  ...
  const file = await prisma.fileAsset.findUnique({ where: { id: fileId } }); if (!file) return NextResponse.json({ error: "文件不存在" }, { status: 404 }); const permission = await projectFileAccess(user.id, file.projectId); if (!permission.canManage && file.creatorId !== user.id) return NextResponse.json({ error: "没有修改文件的权限" }, { status: 403 });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ fileId: string }> }) {
  const user = await fileUser(request); if (!user) return NextResponse.json({ error: "请先登录" }, { status: 401 }); const { fileId } = await params;
  const file = await prisma.fileAsset.findUnique({ where: { id: fileId } }); if (!file) return NextResponse.json({ error: "文件不存在" }, { status: 404 }); const permission = await projectFileAccess(user.id, file.projectId); if (!permission.canManage && file.creatorId !== user.id) return NextResponse.json({ error: "没有删除文件的权限" }, { status: 403 });
  const permanent = new URL(request.url).searchParams.get("permanent") === "1";
  if (permanent) { if (!permission.canManage) return NextResponse.json({ error: "只有项目管理者可以永久删除文件" }, { status: 403 }); const versions = await prisma.fileVersion.findMany({ where: { fileId }, select: { objectKey: true } }); await Promise.all(versions.map(version => deleteObject(version.objectKey).catch(() => undefined))); await prisma.fileAsset.delete({ where: { id: fileId } }); return NextResponse.json({ ok: true, permanent: true }); }
