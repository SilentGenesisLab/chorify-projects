...
const inputSchema = z.object({ projectId: z.string().min(1), parentId: z.string().nullable().optional(), name: z.string().trim().min(1).max(120).refine((name) => !/[\\/:*?"<>|]/.test(name), "文件夹名称包含非法字符") });
export async function POST(request: Request) {
  ...
  const permission = await projectFileAccess(user.id, input.data.projectId); if (!permission.canWrite) return NextResponse.json({ error: "没有创建文件夹的权限" }, { status: 403 });
