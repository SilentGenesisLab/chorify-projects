...
async function submit(request: Request, { params }: { params: Promise<{ taskId: string }> }) {
  ...
  const report = await prisma.$transaction(async (tx) => { const created = await tx.workReport.create({ data: { taskId, authorId: userId, ...parsed.data } }); await tx.task.update({ where: { id: taskId }, data: { status: "PENDING_ACCEPTANCE", startedAt: task.startedAt || new Date() } }); await tx.auditLog.create({ data: { userId, projectId: task.projectId, actorType: "USER", action: "SUBMIT_REPORT", resource: "TASK", resourceId: taskId, channel: "WEB", metadata: { projectId: task.projectId, result: "SUCCESS" } } }); return created; });
