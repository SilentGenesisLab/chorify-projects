...
export async function GET(request: Request, { params }: { params: Promise<{ taskId: string }> }) {
  const auth = await authenticateApi(request); const { taskId } = await params;
  ...
  if (!task || (auth.projectId && task.projectId !== auth.projectId) || !await prisma.projectMember.findUnique({ where: { projectId_userId: { projectId: task.projectId, userId: auth.userId } } })) return NextResponse.json({ error: "无权访问该任务" }, { status: 403 });
