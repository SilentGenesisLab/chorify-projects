...
export async function GET(request: Request, { params }: { params: Promise<{ taskId: string }> }) {
  ...
  if (!task) return NextResponse.json({ error: "无权访问该任务" }, { status: 403 });
  if (!await tokenCanAccessProject(auth, task.projectId)) {
    await auditApiCall({ auth, action: "READ_TASK_CONTEXT", resource: "TASK", resourceId: taskId, projectId: task.projectId, result: "DENIED", request });
    return NextResponse.json({ error: "无权访问该任务" }, { status: 403 });
  }
  await auditApiCall({ auth, action: "READ_TASK_CONTEXT", resource: "TASK", resourceId: taskId, projectId: task.projectId, request });
