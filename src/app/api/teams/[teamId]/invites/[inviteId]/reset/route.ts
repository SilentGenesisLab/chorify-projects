...
export async function POST(request: NextRequest, { params }: { params: Promise<{ teamId: string; inviteId: string }> }) {
  ...
  const invite = await prisma.$transaction(async (tx) => {
    const updated = await tx.teamInvite.update({ where: { id: inviteId }, data: { prefix: token.prefix, tokenHash: token.tokenHash, tokenCiphertext: encryptTeamInviteToken(token.token), useCount: 0, revokedAt: null, expiresAt: new Date(Date.now() + 7 * 86_400_000) } });
