...
const appSecretCipherKey = (purpose: string, secret = process.env.AUTH_SECRET) => {
  if (!secret) throw new Error("AUTH_SECRET is required to protect application secrets");
  return createHash("sha256").update(`chorify-app-secret:${purpose}:${secret}`).digest();
};

export function encryptAppSecret(value: string, purpose: string, secret?: string) {
  const iv = randomBytes(12);
  const cipher = createCipheriv("aes-256-gcm", appSecretCipherKey(purpose, secret), iv);
  const ciphertext = Buffer.concat([cipher.update(value, "utf8"), cipher.final()]);
  return `v1.${iv.toString("base64url")}.${cipher.getAuthTag().toString("base64url")}.${ciphertext.toString("base64url")}`;
}

export function decryptAppSecret(payload: string, purpose: string, secret?: string) {
  try {
    const [version, iv, tag, ciphertext] = payload.split(".");
    if (version !== "v1" || !iv || !tag || !ciphertext) return null;
    const decipher = createDecipheriv("aes-256-gcm", appSecretCipherKey(purpose, secret), Buffer.from(iv, "base64url"));
