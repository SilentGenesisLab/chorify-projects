...
async function main() {
  ...
  const version = await prisma.version.create({ data: { projectId: project.id, name: "V0.9", goal: "完成协作内测闭环", status: VersionStatus.DEVELOPING, plannedAt: new Date("2026-09-05") } });
  const requirement = await prisma.requirement.create({ data: { code: "REQ-12", projectId: project.id, title: "任务提交与验收闭环", description: "成员可以提交结构化工作汇报并由指定验收人处理", acceptanceCriteria: "提交人与验收人分离，完整保留操作记录", priority: Priority.HIGH, status: "IN_PROGRESS", targetVersionId: version.id } });
  ...
  await prisma.fileAsset.create({ data: { projectId: project.id, name: "权限矩阵-v2.xlsx", mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", size: 24836, tags: ["权限", "V0.9"] } });
